...
public class Block extends UnlockableContent implements Senseable {
    ...
    /** 该方块是否会被动被子弹命中，除非被玩家或AI显式指定为目标 */
    public boolean underBullets;
    /** 该方块是否可旋转 */
    public boolean rotate;
    /** 在上方那个接口被启动的时候再起被定义：方块逻辑上被旋转，但贴图不被旋转 */
    public boolean rotateDraw = true;
    /** 方块可旋转式关闭的时候，是否将他它强制锁定角度为0度 */
    ...
    /** 仅对静态方块（无实体，如装饰墙）有效。若为 true ，方块附加数据（如存储的物品）会保存在地图文件中 */
    public boolean saveData;
    /** 若为 false ，无法通过右键手动拆除 */
    public boolean breakable;
    /** 若为 true ，单位踩过时会自动破坏该方块 */
    public boolean unitMoveBreakable;
    /** whether to add this block to brokenblocks */
    public boolean rebuildable = true;
    /**
     * if true, this logic-related block can only be used with privileged processors
     * (or is one itself)
     */
    ...
    /**
     * whether to automatically reset enabled status after a logic block has not
     * interacted for a while.
     */
    ...
    /**
     * if true, this block updates in payloads in units regardless of the
     * experimental game rule
     */
    public boolean alwaysUpdateInUnits = false;
    /** if true, this block can be picked up in payloads */
    public boolean canPickup = true;
    /**
     * if false, only incinerable liquids are dropped when deconstructing;
     * otherwise, all liquids are dropped.
     */
    ...
    /**
     * if true, this block cannot be mined by players. useful for annoying things
     * like sand.
     */
    ...
    /**
     * Health per square tile that this block occupies; essentially, this is
     * multiplied by size * size. Overridden if health is > 0. If <0, the default is
     * 40.
     */
    ...
    /**
     * if true, destroyBullet is spawned on the block's team instead of Derelict
     * team
     */
    ...
    /**
     * Multiplier of damage dealt to this block by tanks. Does not apply to
     * crawlers.
     */
    ...
    /**
     * Special flag; if false, floor will be drawn under this block even if it is
     * cached.
     */
    ...
    /**
     * The block group. Unless {@link #canReplace} is overridden, blocks in the same
     * group can replace each other.
     */
    ...
    /**
     * If true, this block does not have pointConfig with a transform called on map
     * resize.
     */
    ...
    /**
     * Environmental flags that are *all* required for this block to function. 0 =
     * any environment
     */
    public int envRequired = 0;
    /**
     * The environment flags that this block can function in. If the env matches any
     * of these, it will be enabled.
     */
    public int envEnabled = Env.terrestrial;
    /**
     * The environment flags that this block *cannot* function in. If the env
     * matches any of these, it will be *disabled*.
     */
     ...
     * Building must implement Ranged.
     */
    public boolean attacks = false;
    /**
     * If true, this block is mending-related and can be suppressed with special
     * units/missiles.
     */
    ...
    /**
     * How much fog this block uncovers, in tiles. Cannot be dynamic. <= 0 to
     * disable.
     */
    public int fogRadius = -1;

    /**
     * The sound that this block makes while idle. Uses one sound loop for all
     * blocks.
     */
    ...
    /**
     * Time to build this block in ticks. If this value is <0, it is calculated
     * dynamically.
     */
    ...
    /**
     * If true, this block deconstructs immediately. Instant deconstruction implies
     * no resource refund.
     */
    public boolean instantDeconstruct = false;
    /**
     * If true, this block constructs immediately. This implies no resource
     * requirement, and ignores configs - do not use, this is for performance only!
     */
    public boolean instantBuild = false;
    /**
     * If true, this block can be placed even in "dark" areas. Only used for editor
     * static walls.
     */
    ...
    /**
     * Override for research cost. Uses multipliers above and building requirements
     * if not set.
     */
    ...
    /**
     * Building that is created for this block. Initialized in init() via
     * reflection. Set manually if modded.
     */
    ...
    /**
     * Regions indexes from icons() that are rotated. If either of these is not -1,
     * other regions won't be rotated in ConstructBlocks.
     */
    ...
    public void drawShadow(Tile tile) {
        ...
        Draw.rect(
                variants == 0 ? customShadowRegion
                        : variantShadowRegions[Mathf.randomSeed(tile.pos(), 0,
                                Math.max(0, variantShadowRegions.length - 1))],
                tile.drawx(), tile.drawy());
        Draw.color();
    }
    ...
    public float percentSolid(int x, int y) {
        ...
        return tile.getLinkedTilesAs(this, tempTiles)
                .sumf(other -> !other.floor().isLiquid ? 1f : 0f) / length / width;
    }
    ...
    /**
     * Draws a region to overlay a specific side of this block. This method makes
     * sure it is placed at the edge of the side.
     */
        ...
        if ((consumesPower || outputsPower) && hasPower && connectedPower) {
            ...
            if (tile != null) {
                PowerNode.getNodeLinks(tile, this, player.team(), other -> {
                    ...
                    node.drawLaser(x * tilesize + offset, y * tilesize + offset, other.x, other.y, size,
                            other.block.size);
                ...
                BeamNode.getNodeLinks(tile, this, player.team(), other -> {
                    ...
                    Draw.color(node.laserColor1, Renderer.laserOpacity * 0.5f);
                    node.drawLaser(other.x, other.y, x * tilesize + offset, y * tilesize + offset, size,
                            other.block.size);
    ...
    public void setStats() {
        ...
        stats.add(Stat.size, "@x@", length, width);
    ...
    public void addLiquidBar(Liquid liq) {
        addBar("liquid-" + liq.name, entity -> !liq.unlockedNow() ? null
                : new Bar(
                        () -> liq.localizedName,
                        liq::barColor,
    ...
    public <T extends Building> void addLiquidBar(Func<T, Liquid> current) {
        addBar("liquid", entity -> new Bar(
                () -> current.get((T) entity) == null || entity.liquids.get(current.get((T) entity)) <= 0.001f
                        ? Core.bundle.get("bar.liquid")
                        : current.get((T) entity).localizedName,
                () -> current.get((T) entity) == null ? Color.clear : current.get((T) entity).barColor(),
                () -> current.get((T) entity) == null ? 0f
                        : entity.liquids.get(current.get((T) entity)) / liquidCapacity));
    }
    ...
    public void setBars() {
        ...
        if (consPower != null) {
            ...
            addBar("power", entity -> new Bar(
                    ...
                    () -> Mathf.zero(consPower.requestedPower(entity))
                            && entity.power.graph.getPowerProduced() + entity.power.graph.getBatteryStored() > 0f ? 1f
                                    : entity.power.status));
        }
        ...
        // liquids added last
        if (hasLiquids) {
            // TODO liquids need to be handled VERY carefully. there are several potential
            // possibilities:
    ...
    public boolean canReplace(Block other) {
        ...
        if (other.privileged)
            return false;
        return other.replaceable && (other != this || (rotate && quickRotate))
                && ((this.group != BlockGroup.none && other.group == this.group) || other == this) &&
                (length * width == other.size || (length * width >= other.size
                        && ((subclass != null && subclass == other.subclass) || group.anyReplace)));
    }

    /**
     * @return a possible replacement for this block when placed in a line by the
     *         player.
     */
    ...
    public boolean configSenseable() {
        return configurations.containsKey(Item.class) || configurations.containsKey(Liquid.class)
                || configurations.containsKey(UnlockableContent.class) ||
    ...
    /**
     * Called when a new build plan is created in the player's queue. Blocks can
     * maintain a reference to this plan and add configs to it later.
     */
    ...
    /**
     * this is a different method so subclasses can call it even after overriding
     * the base
     */
    ...
    /**
     * sets {@param out} to the index-th side outside of this block, using the given
     * rotation.
     */
    public void nearbySide(int x, int y, int rotation, int index, Point2 out) {
        int cornerX = x - (length - 1) / 2, cornerY = y - (width - 1) / 2, s = size, l = length, w = width;
        switch (rotation) {
            case 0 -> out.set(cornerX + l, cornerY + index);
            case 1 -> out.set(cornerX + index, cornerY + w);
    ...
    /**
     * @return special icons to outline and save with an -outline variant. Vanilla
     *         only.
     */
    ...
    protected TextureRegion[] icons() {
        ...
        return teamRegion.found() && minfo.mod == null ? new TextureRegion[] { r, teamRegions[Team.sharded.id] }
                : new TextureRegion[] { r };
    }
    ...
    public Rect bounds(int x, int y, Rect rect) {
        if (length >= width) {
            return rect.setSize(length * tilesize).setCenter(x * tilesize + offset, y * tilesize + offset);
        } else {
            return rect.setSize(width * tilesize).setCenter(x * tilesize + offset, y * tilesize + offset);
        }
    }
    ...
    public boolean supportsEnv(int env) {
        return (envEnabled & env) != 0 && (envDisabled & env) == 0
                && (envRequired == 0 || (envRequired & env) == envRequired);
    }

    /**
     * Called to set up configuration UI in the editor. {@link #editorConfigurable}
     * must be true.
    ...
    /**
     * @return the config value returned when this block is picked on a certain
     *         tile. This is only called for non-buildings.
     */
    ...
    public void removeConsumer(Consume cons) {
        if (consumers.length > 0) {
            throw new IllegalStateException(
                    "You can only remove consumers before init(). After init(), all consumers have already been initialized.");
        }
     ...
     * 
     * @param powerPerTick The amount of power which is required each tick for 100%
     *                     efficiency.
        ...
        try {
            ...
            while (buildType == null && Block.class.isAssignableFrom(current)) {
                ...
                if (type != null) {
                    ...
                    Constructor<? extends Building> cons = (Constructor<? extends Building>) type
    ...
    public ItemStack[] researchRequirements() {
        if (researchCost != null)
            return researchCost;
        if (researchCostMultiplier <= 0f)
            return ItemStack.empty;
        ItemStack[] out = new ItemStack[requirements.length];
        for (int i = 0; i < out.length; i++) {
            int quantity = Mathf.round(60 * researchCostMultiplier + Mathf.pow(requirements[i].amount, 1.11f) * 20
                    * researchCostMultiplier * researchCostMultipliers.get(requirements[i].item, 1f), 10);
    ...
    public void getDependencies(Cons<UnlockableContent> cons) {
        ...
        for (var c : consumeBuilder) {
            ...
            // TODO: requiring liquid dependencies is usually a bad idea, because there is
            // no reason to pump/produce something until you actually need it.
            /*
    ...
    public void postInit() {
        // usually, an empty set of planets is a configuration error. auto-assign based
        // on requirements
    ...
    public void init() {
        ...
        if (health == -1) {
            ...
            health = round ? Mathf.round(width * length * scaledHealth, 5) : (int) (width * length * scaledHealth);
        }

        if (length >= width) {
            clipSize = Math.max(clipSize, length * tilesize);
        } else {
            clipSize = Math.max(clipSize, width * tilesize);
        }
        ...
        if (hasLiquids && drawLiquidLight) {
            emitLight = true;
            if (length >= width) {
                lightClipSize = Math.max(lightClipSize, length * 30f * 2f);
            } else {
                lightClipSize = Math.max(lightClipSize, width * 30f * 2f);
            }
        }
    ...
    public void load() {
        ...
        for (Team team : Team.all) {
            teamRegions[team.id] = teamRegion.found() && team.hasPalette
                    ? Core.atlas.find(name + "-team-" + team.name, teamRegion)
                    : teamRegion;
        }
    ...
    public void createIcons(MultiPacker packer) {
        ...
        if (outlineIcon) {
            AtlasRegion atlasRegion = (AtlasRegion) gen[outlinedIcon >= 0 ? Math.min(outlinedIcon, gen.length - 1)
                    : gen.length - 1];
        ...
        if (gen.length > 1) {
            ...
            if (gen[0] != null)
                packer.add(PageType.main, "block-" + name + "-full", Core.atlas.getPixmap(gen[0]));
        }
    ...
    /**
     * Fills the specified array with the list of configuration options this block
     * has. Only used for plans.
     */
