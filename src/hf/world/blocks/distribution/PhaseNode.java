package hf.world.blocks.distribution;

import hf.world.drawer.DrawPhaseNode;
...
public class PhaseNode extends ItemBridge {
    ...
    public boolean linkValid(Tile tile, Tile other, boolean checkDouble) {
        if (other == null || tile == null || other == tile)
            return false;
        if (!tile.within(other, (range + size) * tilesize))
            return false;
        return ((other.block() == tile.block() && tile.block() == this)
                || (!(tile.block() instanceof PhaseNode) && other.block() == this))
    ...
    public boolean overlaps(@Nullable Tile src, @Nullable Tile other) {
        if (src == null || other == null)
            return true;
        return Intersector.overlaps(Tmp.cr1.set(src.worldx() + offset, src.worldy() + offset, range * tilesize),
    ...
    public class PhaseNodeBuild extends ItemBridgeBuild {
        ...
        public void doDump() {
            if (hasItems) {
                for (int i = 0; i < items.total() / transportTime; i++)
                    dumpAccumulate();
            }
            if (hasLiquids)
