package hf.world.blocks.liquid;
...
import hf.world.drawer.DrawLiquidDriver;
import hf.world.entities.bullet.LiquidMassDriverBolt;
...
public class LiquidMassDriver extends Block {
    ...
    public LiquidMassDriver(String name) {
        ...
        // point2 is relative
        config(Point2.class, (LiquidMassDriverBuild tile,
                Point2 point) -> tile.link = Point2.pack(point.x + tile.tileX(), point.y + tile.tileY()));
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid) {
        ...
        if (selected == null || selected.block != this || !selected.within(x * tilesize, y * tilesize, range))
            return;

        // if so, draw a dotted line towards it while it is in range
        float sin = Mathf.absin(Time.time, 6f, 1f);
        Tmp.v1.set(x * tilesize + offset, y * tilesize + offset).sub(selected.x, selected.y)
                .limit((size / 2f + 1) * tilesize + sin + 0.5f);
    ...
    public class LiquidMassDriverBuild extends Building {
                ...
                if (liquidTotal() >= minDistribute &&
                    ...
                    if (reloadCounter <= 0.0001f) {
                        ...
                        if (other.currentShooter() == this &&
                                ...
                                && Angles.near(other.rotation, targetRotation + 180f, 2f)) {
                            // actually fire
                            fire(other);
        ...
        public double sense(LAccess sensor) {
            if (sensor == LAccess.progress)
                return Mathf.clamp(1f - reloadCounter / reload);
