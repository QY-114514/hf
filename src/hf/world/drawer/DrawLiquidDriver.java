package hf.world.drawer;

import arc.Core;
import arc.graphics.g2d.Draw;
import arc.math.Angles;
import hf.world.blocks.liquid.LiquidMassDriver;
import hf.world.blocks.liquid.LiquidMassDriver.LiquidMassDriverBuild;
        ...
        public void drawDriver(LiquidMassDriver block, LiquidMassDriverBuild build) {
                ...
                Drawf.shadow(Core.atlas.find(block.name + "-region"),
                                build.x + Angles.trnsx(build.rotation + 180, build.reloadCounter * block.knockback)
                                                - (block.size / 2f),
                                build.y + Angles.trnsy(build.rotation + 180, build.reloadCounter * block.knockback)
                                                - (block.size / 2f),
                                build.rotation - 90);
